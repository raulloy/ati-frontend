  ...
  return {
    resource:
      request[1] == undefined
        ? false
        : request[1].includes('?')
        ? request[1].slice(0, request[1].indexOf('?'))
        : request[1].slice(0, request[1].length),
    page:
      request[1] == undefined
        ? false
        : request[1].includes('?')
        ? request[1].slice(request[1].indexOf('=') + 1, request[1].indexOf('&'))
        : 1,
    id: request[2],
    action: request[3],
  };
...
export function PaginationButton(
  ...
  this.render = (container = document.getElementById('main-container')) => {
